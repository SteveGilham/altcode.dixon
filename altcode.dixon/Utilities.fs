...
module Utilities =
  ...
  let FindRuleClass assembly rule =
    ...
    let rule = source.GetTypes() |> Seq.find (fun t -> t.Name = rule)
  ...
  [<assembly:SuppressMessage("Microsoft.Reliability",
                             "CA2001:AvoidCallingProblematicMethods", Scope = "member",
                             Target =
  ...
  [<assembly:SuppressMessage("Microsoft.Naming",
                             "CA1709:IdentifiersShouldBeCasedCorrectly", Scope = "member",
                             Target =
  ...
  type Microsoft.FxCop.Sdk.Member with
    member self.HasAttribute typeName =
      self.Attributes |> Seq.exists (fun a -> a.Type.FullName = typeName)

    member self.IsFSharpCode =
      self.IsNotNull && match self with
                        | :? TypeNode as t ->
