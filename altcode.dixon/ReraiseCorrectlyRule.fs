...
type ReraiseCorrectlyRule =
  class
    inherit BaseIntrospectionRule

    // Default constructor as required
    new() =
      { inherit BaseIntrospectionRule("ReraiseCorrectly", "altcode.dixon.Dixon.Design",
      ...
      | :? Method as fn ->
          if fn.IsFSharpCode then
            ...
            let throws =
              ...
              |> Seq.mapi
                   (fun index instr ->
                     if instr.OpCode = OpCode.Throw then Some index else None)
              |> Seq.choose id
              |> Seq.toList
            ...
            if throws
               |> List.exists (fun i ->
                    if indexable.[i - 1].OpCode = OpCode.Newobj then
                      ...
                      match (stloc indexable.[i - 4], ldloc indexable.[i - 3],
                             stloc indexable.[i - 2], ldloc indexable.[i - 1]) with
    ...
    override self.VisitExpression(expression : Expression) =
      if expression.IsNotNull && expression.NodeType = NodeType.Call then
        ...
        | :? MemberBinding as b ->
            if b.BoundMember.FullName.StartsWith
                 ("Microsoft.FSharp.Core.Operators.raise<", StringComparison.Ordinal) then
